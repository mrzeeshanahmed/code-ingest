    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      stream.on("data", (chunk: string | Buffer) => {
        ...
        if (chunkBuffer.length >= remaining) {
          chunks.push(chunkBuffer.slice(0, remaining));
          collected += remaining;
          stream.destroy();
        } else {
          chunks.push(chunkBuffer);
          collected += chunkBuffer.length;
          if (collected >= sampleSize) {
            stream.destroy();
          }
        }
    ...
    const dataPromise = new Promise<void>((resolve, reject) => {
      ...
      stream.on("data", (chunk: string | Buffer) => {
        ...
        if (targetSize > 0 && offset < targetSize) {
          const writable = Math.min(chunkBuffer.length, targetSize - offset);
          if (writable > 0) {
            chunkBuffer.copy(bufferStore, offset, 0, writable);
            offset += writable;
          }
        }
