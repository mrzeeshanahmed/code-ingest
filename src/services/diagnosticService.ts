  ...
  async checkNetworkConnectivity(): Promise<DiagnosticResult> {
    ...
    const outcomes = await Promise.all(
      testUrls.map(async (url) => {
        try {
          await this.networkTester(url);
          return true;
        } catch (error) {
          this.logger.warn("diagnostics.network.endpointFailed", { url, message: (error as Error).message });
          return false;
        }
      })
    );
    ...
    if (successRate === 0) {
      status = "fail";
      suggestion = "No network connectivity detected. Check your internet connection.";
    } else if (successRate < 0.5) {
      status = "warning";
      suggestion = "Limited network connectivity. Some remote features may not work.";
    }
    ...
    const result: DiagnosticResult = {
      ...
      status,
      message: `Network connectivity: ${successCount}/${testUrls.length} endpoints reachable`,
      metadata: { successRate, testedEndpoints: testUrls.length }
    };
