...
async function importModuleScripts(document: Document, htmlFilePath: string): Promise<void> {
  ...
  for (const script of scripts) {
    ...
    await bundleAndExecuteModule(window, absolutePath);
  }
}

async function bundleAndExecuteModule(window: Window, entryPath: string): Promise<void> {
  ...
  if (!output) {
    throw new Error(`Failed to bundle module at ${entryPath}`);
  }

  const script = window.document.createElement("script");
  script.textContent = output.text;
