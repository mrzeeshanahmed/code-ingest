    ...
    if (selected === "Show Details") {
      ...
      try {
        ...
        const openIssue = await vscode.window.showInformationMessage(
          "Error report copied to clipboard. Attach it to your issue tracker of choice if you want to file a report.",
          "OK"
        );

        if (openIssue === "OK") {
          this.outputChannel.appendLine(`Prepared local error report for ${errorId}.`);
        }
      } catch (e) {
