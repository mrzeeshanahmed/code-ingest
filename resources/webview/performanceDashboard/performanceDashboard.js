...
class PerformanceDashboard {
  ...
  postMessageSafely(message, context) {
    try {
      if (this.vscode && typeof this.vscode.postMessage === "function") {
        this.vscode.postMessage(message);
      } else {
        console.warn(`PerformanceDashboard: vscode.postMessage unavailable (${context})`);
      }
    } catch (error) {
      console.error(`PerformanceDashboard: failed to post message (${context})`, error instanceof Error ? error.message : String(error));
    }
  }
  ...
  requestInitialData() {
    this.requestMetricsUpdate();
    this.postMessageSafely({ type: "requestHistorical" }, "requestHistorical");
  }

  requestMetricsUpdate() {
    this.postMessageSafely({ type: "requestMetrics" }, "requestMetrics");
  }

  exportPerformanceReport() {
    this.postMessageSafely({ type: "exportReport" }, "exportReport");
  }

  toggleRealTimeUpdates(enabled) {
    this.isRealTimeEnabled = enabled;
    this.postMessageSafely({ type: "toggleRealTime", payload: enabled }, "toggleRealTime");
  }
